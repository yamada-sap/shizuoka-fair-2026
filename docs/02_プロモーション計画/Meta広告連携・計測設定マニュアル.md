...
### 4.1 Metaピクセル（ベースコード）の取得とGTM設定
従来の「カスタムHTML」に直接JavaScriptを貼り付ける手法に加え、現在はGTMの**「カスタムテンプレート」**機能を利用する設定方法が最も安全で推奨されています。

#### 【推奨：最新方式】GTMカスタムテンプレートによる設定
1. GTM管理画面で「**テンプレート**」➔ タグテンプレートの「**ギャラリーを検索**」をクリック。
2. 「**Meta Pixel** (提供: *facebookarchive*)」を検索し、「ワークスペースに追加」を実行。
3. 「**タグ**」➔「新規」をクリックし、タグタイプとして「**Meta Pixel**」を選択。
4. 設定画面に「データセットID（ピクセルID）」を入力するだけでノーコードで設定完了。
5. トリガーに `Initialization - All Pages` を紐づけて保存。

---

## 4. MetaピクセルとGTMの連携・計測設定 (2/4)
...
## 4. MetaピクセルとGTMの連携・計測設定 (3/4)

### 4.2 GTMでのカスタムHTMLタグ設定（従来方式）
- **タグタイプ**：`カスタムHTML` を選択。
- **HTML欄**：前スライドのベースコードをそのまま貼り付けます。
- **トリガー**：`Initialization - All Pages`（初期化 - すべてのページ）を設定。
- **設定名**：`Meta - Pixel - BaseCode` と命名して保存。

---

## 4. MetaピクセルとGTMの連携・計測設定 (4/4)
...
### 4.3 標準イベントの設定（コンバージョン計測）
特定の重要なアクションをコンバージョンとして計測するため、GTMでイベント用のタグを設定します。推奨テンプレートまたはカスタムHTMLタグを作成します。
...
## 6. コンバージョンAPI（CAPI）の連携 (2/3)

### 6.1 実装方式の選定（最新の推奨選択肢）

1. **【推奨：ノーコード】Conversions API Gateway（CAPIゲートウェイ）**
   - Metaが提供するクラウドホスト型の自動API連携システム。AWSなどのクラウド上に数クリックでCAPI環境を自動構築でき、開発コストが大幅に抑えられます。
2. **【パートナー連携】WordPress/Shopify等の標準機能連携**
   - プラグイン（*Pixel Your Site*等）にアクセストークンを貼り付けるだけのノーコード連携。
3. **【高度な連携】GTMサーバーサイドコンテナによる連携**
   - Webコンテナで収集したイベントデータをGCPなどのクラウドサーバー経由でMetaへ中継送信します。
4. **【直接実装】Webサーバーからの直接API通信**
   - バックエンド（PHPやNode.js等）から直接HTTPSリクエストでMetaエンドポイントへPOSTします。

---

## 6. コンバージョンAPI（CAPI）の連携 (3/3)
...
### 6.2 重複排除（Deduplication）の最新コード仕様
ブラウザとAPIの双方から同一コンバージョンが送信された場合の二重計上を防ぐため、ユニークな `event_id` を付与し重複排除を行います。

- **【ブラウザ送信（GTMカスタムHTML時のJavaScript）】**
  Metaの最新引数仕様に基づき、**第3引数にカスタムデータオブジェクト（空でも可）、第4引数にイベントメタデータオブジェクト（eventIDを含む）**を正確に渡します。
  ```javascript
  fbq('track', 'Contact', {}, {eventID: 'contact_order_12345'});
  ```
- **【サーバー送信（CAPI）】**
  CAPIの送信パラメータ（`event_id` プロパティ）に、ブラウザ側と完全に一致する `"contact_order_12345"` を含めて送信します。
- **効果**：Metaサーバー側で同一IDを検知すると、重複データを自動的に統合・破棄します。
...
過去に他社（旧代理店・旧事務局）が運用していた場合、各種アセットが他社のビジネスマネージャに登録されているため、移行実務が必要です。
