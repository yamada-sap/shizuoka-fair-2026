...
本計画書は、「産業フェアしずおか2026」のプロモーションにおいて、割り当てられた予算（100,000円）を最適に運用し、Yahoo!ディスプレイ広告（YDA：旧YDN）および提携するLINEの広告枠を活用して幅広いターゲット層にアプローチし、公式サイトへの集客と来場者数を最大化するための「詳細広告配信プラン」です。
...
- **総予算上限**：100,000円
- **想定表示回数（インプレッション数）**：1,111,111回（想定平均シナリオ）
- **想定クリック数**：3,500回 〜 6,875回（想定平均: 5,000回 / 想定平均CTR: 0.45%）
- **想定クリック単価（CPC）**：14.5円 〜 28.6円（想定平均: 20.0円）
- **目標オフライン来場世帯数**：2組 〜 10組（想定CVR: 0.05% 〜 0.15%、想定平均: 5組）
...
```mermaid
graph TD
    YDA[Yahoo!ディスプレイ広告 <br>予算: 100,000円] --> GroupA[グループA: 子育てファミリー層 <br>配分比率: 60%]
...
Yahoo!ディスプレイ広告（YDA）は、登録した「画像 ＋ タイトル（見出し） ＋ 説明文」の組み合わせで表示されます。ターゲットグループ別に以下のブラッシュアップ済みコピードラフトを入稿します。

### 【グループA：ファミリー層向け】入稿ドラフト（各5パターン）

#### ① タイトル（全角20文字以内）

1. 【入場無料】11/28・29親子体験フェス
2. 雨でも遊べる！ツインメッセ
3. 無料！プラモ＆木工体験ブース
4. 静岡の体験とグルメが大集合
5. 11/28・29家族でお出かけ
...
- **「1秒認知」と片手操作UI/UX**：
  - 子育て世代がベビーカーを押しながらスマホを片手操作（親指タップ）で流し見するシーンを想定し、広告コピーのタイトル（全角20文字以内）や見出しは**「1秒で頭に入る大きな文字サイズとシンプルなレイアウト」**を徹底します。
  - スマートフォンのLINE NEWSやYahoo!トップ等での極小バナー面においても、認知摩擦を低減するため「1メッセージ・1ビジュアル」に要素を絞り込みます。
  - 特設サイト遷移後の離脱を防ぐため、送客LPのファーストビューは情報量を整理し、ユーザーが求める「アクセス・駐車場」「入場無料」「開催概要」を簡潔かつ大きく表示する設計とします。
