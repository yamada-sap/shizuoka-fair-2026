...
静岡県内の主要8市町に居住する子育てファミリー層へピンポイントにアプローチし、週末の来場獲得コスト（CAC/CPA）を最小化しつつ、目標表示回数を確保すること。
...
- 冒頭3秒のテロップや視覚情報をMetaのAIが解析し、ターゲット層へ自動最適マッチング。

### ③ Advantage+ クリエイティブ最適化の有効化
- アスペクト比のトリミング調整やコントラスト補正を有効化しアクション率を最大化。

### ④ パートナーシップ広告の連携
- 提携インフルエンサー（`@shizuoka_info`, `@shizuokaosanponikki`）の投稿をパートナーシップコード経由でブースト配信。
...
- **対象アセット仕様**：1080px × 1920px (9:16)、25秒
...
- **構成詳細（25秒）**：
  ...
  - **[4-10秒]**
    - 映像：伝統工芸の職人さんが優しく教える木工体験・手仕事の実演シーン。
...
| **想定CPM** | 500円 | **433円** | 380円 | Meta Ads子育て層ターゲティング平均 |
| **想定表示回数** | 200,000回 | **230,947回** | 263,157回 | 予算 ÷ (CPM ÷ 1,000) |
| **想定CTR** | 1.2% | **1.6%** | 2.0% | パートナーシップ広告過去平均 |
| **想定クリック数** | 2,400回 | **3,695回** | 5,263回 | 表示回数 × CTR |
| **想定平均CPC** | 41.7円 | **27.1円** | 19.0円 | 予算 ÷ クリック数 |
...
- [ ] **【パートナーシップコード連携】** 提携インフルエンサーからの連携コード入力が正常に完了しているか？
- [ ] **【セーフエリア配慮】** 9:16縦型動画の中央60%内にメインテキストが収まっているか？
...
print("INSTAGRAM AD PLAN CLEANED")
